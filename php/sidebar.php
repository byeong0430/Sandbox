<div class="col-12">
    <!-- Profile -->
    <div class="row">
        <div class="col-12 inner-shell" id="profile-div">
            <div class="speech-bubble">
                <span id="speech">
                </span>
            </div>
            <div class="profile-head">
                <img id="profile-img" src="img/profile.jpg" alt="Girl in a jacket" style="vertical-align:middle">
                <div class="description">
                    <span class="profile" id="profileName"><h2>Byeong Kim</h2></span>
                    <span class="profile" id="profileDescription">Data Geek</span>
                </div>
            </div>
        </div>
    </div>
    <!-- Profile -->

    <!-- Social media -->
    <div class="row">
        <div class="col-12 inner-shell" id="socialmedia-wrapper">
            <i class="fab fa-facebook-square fa-3x" style="color: grey;"></i>
            <i class="fab fa-instagram fa-3x" style="color: grey;"></i>
            <i class="fab fa-linkedin fa-3x" style="color: grey;"></i>
            <i class="fab fa-github-square fa-3x" style="color: grey;"></i>
        </div>
    </div>
    <!-- Social media -->

    <!-- Process window -->
    <div class="row">
        <div class="col-12 inner-shell" id="progressbar-wrapper">
            <h5>Progress Bar</h5>
            <hr>
            <span class="consent-span">
                <i class="progress-icon far fa-question-circle fa-2x" id="consent-question-icon"></i>
                Consent Agreement
            </span>
            <br>
            <br>
            <table class="table">
                <thead>
                    <tr>
                        <th>Selected Locations</th>
                    </tr>
                </thead>
                <tbody class="drop-tbody">
                </tbody>
            </table>
        </div>
    </div>
    <!-- Process window -->
</div>